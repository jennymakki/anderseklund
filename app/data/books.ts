export const books = [
    {
    id: "1",
    title: "Heby från järnvägen",
    description: "En lokalhistorisk bok som skildrar hur Heby stationssamhälle växte fram från slutet av 1800-talet i takt med järnvägen. Den berättar om samhällsbyggandet, vardagslivet och människorna som formade platsen – från tegelbruk och skolor till lokala berättelser.",
    image: "/books/hebyfrånjärnvägen.png",
    price: "199 kr - Frakt kan tillkomma",
  },
  {
    id: "2",
    title: "Tillbakablick Heby - En by i förändring",
    description: "Under 1960-talet förändrades Heby i grunden. Vägarna gick genom centrum, inga hyreshus fanns och tegelbruken var i full drift, vilket gav arbete och formade samhället på den tiden.",
    image: "/books/tillbakablick.png",
    price: "199 kr - Frakt kan tillkomma",
  },
  {
    id: "3",
    title: "Club Shake",
    description: "Ett minne från en tid när Heby var navet för tonåringarna i städer och byar kring Heby. Ett minne av musiken, banden, hysterin och upplevelsen, som dagens 75 åringar fick uppleva i Heby när de var tonåringar.",
    image: "/books/clubshake.png",
    price: "199 kr - Frakt kan tillkomma",
  },
  {
    id: "4",
    title: "Heby AIF Damfotboll på 1970-talet",
    description: "Damfotbollen i Heby tog sina första stapplande steg under 1970-talet. Denna bok bygger på berättelser från de kvinnor och tränare som själva var med och upplevde den tidiga utvecklingen.",
    image: "/books/hebyaifdam.png",
    price: "99 kr - Frakt kan tillkomma",
  },
  {
    id: "5",
    title: "Fotbollskrönika från Heby - 1912-1969",
    description: "En kärleksförklaring till Heby AIF och deras historia 1912-1969, där klubbens utveckling speglar samhällets förändringar - från de tidiga åren till 60-talets omvandling, med kampen om ekonomi som en röd tråd.",
    image: "/books/fotbollskrönika.png",
    price: "199 kr - Frakt kan tillkomma",
  },
  {
    id: "6",
    title: "Heby AIF Herrfotboll 1970-1999",
    description: "En personlig skildring av Heby AIF:s herrfotboll 1970-1999, där A-laget, kulturbärarna och deras berättelser står i centrum. Boken bygger på minnen, intervjuer och matcher snarare än protokoll och fångar fotbollskulturen, gemenskapen och vägen upp till seniorlaget.",
    image: "/books/hebyaif.png",
    price: "199 kr - Frakt kan tillkomma",
  },
];